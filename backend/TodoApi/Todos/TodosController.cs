...
namespace TodoApi.Todos
{
    ...
    public class TodosController : ControllerBase
    {
        ...
        [HttpPut("bulk-status")]
        [SwaggerOperation(OperationId = "UpdateMultipleTodoStatus")]
        public async Task<IActionResult> UpdateMultipleTodoStatus(UpdateMultipleTodoStatusDto dto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            ...
            try
            {
                var userId = GetCurrentUserId();
                var username = GetCurrentUsername();
                var updatedCount = await _todoService.UpdateMultipleTodoStatusAsync(dto.TodoIds, dto.Status, userId, username);

                return Ok(new
                {
                    message = $"Successfully updated status for {updatedCount} todo(s)",
                    updatedCount = updatedCount,
                    requestedCount = dto.TodoIds.Length
                });
            }
            catch (UnauthorizedAccessException)
            {
