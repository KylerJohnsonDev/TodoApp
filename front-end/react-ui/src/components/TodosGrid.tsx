...
export type GridFilterKey = 'all' | 'incomplete' | 'in_progress' | 'complete' | 'undone'
  ...
  <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
    <div className="mx-auto max-w-5xl">
      <div className="gap-4 lg:flex lg:items-center lg:justify-between">
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl">My To-Dos</h2>

        <div className="mt-6 gap-4 space-y-4 sm:flex sm:items-center sm:space-y-0 lg:mt-0 lg:justify-end">
          <div>
            <label htmlFor="grid-filter" className="sr-only mb-2 block text-sm font-medium text-gray-900 dark:text-white">Select order type</label>
            <select defaultValue={gridFilter} onChange={(e) => setGridFilter(e.target.value as GridFilterKey)} id="grid-filter" className="block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500 sm:w-[144px]">
      ...
      <div className="mt-6 flow-root sm:mt-8">
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
          ...
          { displayedTodos.map(todo => (
            <div key={todo.id} className="relative grid grid-cols-2 gap-4 py-6 sm:grid-cols-4 lg:grid-cols-5">
            <div className="col-span-2 content-center sm:col-span-4 lg:col-span-2">
              <span className="text-base font-semibold text-gray-900 dark:text-white">{todo.text}</span>
            </div>
            ...
            <div className="content-center">
              <div className="flex items-center gap-2 lg:justify-center">
                ...
                <p className="text-sm font-medium text-gray-500 dark:text-gray-400">{todo.createdAt}</p>
              </div>
            </div>
